<template>
  ...
  <div class="oversee-stats">
    ...
    <div class="oversee-stats__content">
      ...
      <!-- 图表 -->
      <ve-histogram :data="chartData" :colors="colors" :legend="legend"></ve-histogram>

      <!-- 提示信息 -->
      <div class="oversee-stats__tip-box">
        <div class="oversee-stats__tip-box--title">我的协办</div>
        <ul>
          <li>
...
export default {
  ...
  methods:{
    ...
    setCharts(flag){
      ...
      if(flag == 1){
        chartData = {
          columns: ['类型','所有', '已完成', '未完成'],
          rows: [
            { '类型': '我的主办', '所有': 80, '已完成': 50, '未完成': 8},
            { '类型': '我的协办', '所有': 90, '已完成': 70, '未完成': 10}
          ]
        }
      }else{
        chartData = {
