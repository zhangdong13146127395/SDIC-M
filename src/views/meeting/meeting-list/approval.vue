  ...
  <div class="meeting-list-index">
    ...
    <van-sticky :offset-top="46">
      <van-tabs v-model="active" @click="tabsClick">
        ...
        <van-tab name="yb" title="已办"></van-tab>
      </van-tabs>
    </van-sticky>
    ...
    >
      ...
      <van-cell-group>
        <van-cell :title="`${item.meetingName}`" :label="jointTime(item.beginTime,item.endTime)" v-for="(item, index) in listData" :key="index" @click="view(index)">
...
<script>
import { listMeetingApproval } from '@/api/meeting'
...
export default {
  ...
  methods: {
    ...
    //获取数据
    async onLoad(page) {
      return await listMeetingApproval({ status:this.active, ...page})
    },
